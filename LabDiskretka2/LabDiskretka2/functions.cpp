...
int** MatrixOfDistance(int* Graph, int apex, int size, bool mode)
{
    ...
    while (IsThereZero(matrix_res, apex))
    {
...
void MatrixDosyag(int** matrix, int apex, bool mode)
{
    ...
    OutputMatrix("Матрица достижимости", matrix, apex, mode);
}
...
void GraphYarus(int** matrix, int apex, bool mode)
{
    ...
    for (int i = 0; i < apex; i++)
    {
        ...
        string str = Declination(" ярусa", " ярус", " ярусов", Max);

        *fout << "Для вершины " << i+1 << " есть "<< Max << str << ":";
        for (int d = 0; d < Max; d++)
        {
            *fout << "\n\tЯрусу " << d+1 << " принадлежат: ";
            for (int j = 0; j < apex; j++)
            {
                if (d+1 == matrix[i][j])
                {
...
void GraphCentreNRad(int** matrix, int apex, bool mode, bool centre)
{
    ...
    for (int i = 0; i < apex; i++)
    {
        if (R > matrix[Centr-1][i] && Centr-1 != i)
            R = matrix[Centr-1][i];
    }
    ...
    if (centre)
        *fout << "Центр графа: " << Centr << endl;
    else
        *fout << "Радиус графа: " << R << endl;
